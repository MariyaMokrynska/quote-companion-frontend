...
//   FaHeart,
//   FaPlusSquare,
//   // FaShareAlt, // Share icon commented out
// } from "react-icons/fa";
// import Sidebar from "../components/Sidebar";
// import Navbar from "../components/Navbar";
// import Footer from "../components/Footer";
...
//   const [collections, setCollections] = useState([]);
//   const [selectedCollectionIds, setSelectedCollectionIds] = useState([]); // multiple selections
//   const [newCollectionName, setNewCollectionName] = useState("");
//   const [quoteToAdd, setQuoteToAdd] = useState(null);
...
//           .select("quote_id")
//           .eq("collection_id", filterCollectionId);

//         if (qcError) {
//           console.error("Error fetching quote IDs for collection:", qcError);
//           setQuotes([]);
//           return;
//         }
//         quoteIds = qcData.map((qc) => qc.quote_id);
...
//         .eq("quote_id", id);
//       if (delJoinErr) {
//         console.error("Error deleting from quote_collection:", delJoinErr);
//         return;
//       }
...
//           .single();
//         if (collErr) throw collErr;
//         newCollId = newColl.id;
//         await fetchCollections(userId);
//       }

//       const allCollectionIds = [...selectedCollectionIds];
//       if (newCollId) allCollectionIds.push(newCollId);

//       if (allCollectionIds.length === 0) {
//         alert("Please select or create at least one collection.");
//         return;
//       }

//       console.log("Adding quote to collections:");
//       console.log("Quote ID:", quoteToAdd.id);
//       console.log("Collection IDs:", allCollectionIds);

//       const inserts = allCollectionIds.map((colId) => ({
...
//         .from("quote_collection")
//         .upsert(inserts, { onConflict: ["quote_id", "collection_id"] });

//       if (insertErr) throw insertErr;

//       alert("Quote added to selected collections!");
//       setQuoteToAdd(null);
//       setSelectedCollectionIds([]);
//       setNewCollectionName("");
//       await fetchQuotes(collectionId, userId);
//     } catch (err) {
//       console.error("handleAddToCollectionConfirm error:", err);
//       alert("Failed to add quote to collections.");
//     }
//   };
...
//                         setSelectedCollectionIds(selectedOptions);
//                       }}
//                     >
//                       {collections.map((coll) => (
//                         <option key={coll.id} value={coll.id}>
...
//           initialTags={editingQuote.tags || ""}
//           initialSource={editingQuote.source || ""}
//           defaultCollectionId={null} // no longer single collection
//           onClose={handleCloseEditModal}
//         />
//       )}
//     </div>
